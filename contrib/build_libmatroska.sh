...
build_ebml() {
    ...
    cmake -DCMAKE_OSX_ARCHITECTURES="$ARCHS" -DCMAKE_OSX_DEPLOYMENT_TARGET="$MACOSX_DEPLOYMENT_TARGET" -DDISABLE_PKGCONFIG=YES -DCMAKE_INSTALL_PREFIX="$BUILD_DIR" "$CONTRIB_DIR/libebml"
    make
    make install
...
build_mkv() {
    ...
    cmake -DCMAKE_OSX_ARCHITECTURES="$ARCHS" -DCMAKE_OSX_DEPLOYMENT_TARGET="$MACOSX_DEPLOYMENT_TARGET" -DDISABLE_PKGCONFIG=YES -DCMAKE_INSTALL_PREFIX="$BUILD_DIR" "$CONTRIB_DIR/libmatroska"
    make
    make install
